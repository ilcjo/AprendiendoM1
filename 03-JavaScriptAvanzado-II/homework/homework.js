...
EJEMPLO
const nuevoContador = counter() instancia
nuevoContador()     // 1
nuevoContador()     // 2

const otroContador = counter() crea otra instancia inicializa 0 
otroContador()      // 1
otroContador()      // 2 */
//return function(arr=[])
...
// funcion (x)  retoirna la operacion de ese valor x *2
function cacheFunction(cb) {
  const memoriaCache = {};
  return function (x){
    if(memoriaCache.hasOwnProperty(x)){
     return memoriaCache[x];
    } 
      memoriaCache[x] = cb(x);
        return  memoriaCache[x];
    }
}
var instanciaUno = cacheFunction()
console.log(instanciaUno)
//----------------------------------------
